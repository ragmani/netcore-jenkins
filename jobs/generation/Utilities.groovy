  ...
  def static addBuildSteps(def job, String project, String projectDir = "") {
    ...
    job.with {
      steps {
        ...
        if (project == 'coreclr') {
          shell("${dockerCommand} ./build.sh cross \${config} \${targetArch} cmakeargs \"-DFEATURE_GDBJIT=TRUE -DFEATURE_DBGIPC=0 -DFEATURE_NI_BIND_FALLBACK=false\" stripSymbols -PortableBuild=false \${buildIdOpts} -- ${authorsOpts}")
